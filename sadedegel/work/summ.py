...
class AnnotatedExtractiveSummarizer():
    ...
    def __init__(self, tokenizer, model, k=4, layers=[11], use_CLS_token=False,
                 doEval=True, random_state=None, verbose=False, use_cuda=False):
    ...
    def summarize(self,jsonPath):
        ...
        with torch.no_grad():
            if self.use_cuda:
                outputs = self.model(self._tokens[0].cuda(), self._segments[0].cuda())
            else:
