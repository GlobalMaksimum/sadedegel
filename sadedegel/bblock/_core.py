...
time_re = re.compile(r"\b(?P<hour>1[0-9]|2[0-3]|0?[0-9])[:\.,](?P<minute>[1-5][0-9]|0?[0-9])(?:[:\.,](?P<second>[1-5][0-9]|0?[0-9]))?(?:[:\.,\s+](?P<pm_am>[ap]\.?m\.?))?")
...
date_1_re = re.compile(r'\b(?P<day>0?[0-9]|1[0-9]|2[0-9]|3[01])(?:\s+|[:\-\./])(?P<month>oca(?:k)?|[sş]u(?:bat)?|mar(?:t)?|n[ıi]s(?:an)?|may(?:[ıi]s)?|haz(?:[iı]ran)?|tem(?:muz)?|a[gğ]u(?:stos)?|eyl(?:[uü]l)?|ek[iı](?:m)?|kas(?:[iı]m)?|ara(?:l[iı]k)?)(?:\s+|[:\-\./])(?P<year>\d{4})?',flags=re.IGNORECASE)
date_2_re = re.compile(r'\b(?P<day>0?[0-9]|1[0-9]|2[0-9]|3[01])(?:\s+|[:\-\./])(?P<month>(?:1[0-2]|0?[1-9]))(?:\s+|[:\-\./])(?P<year>\d{4})',flags=re.IGNORECASE)
