...
from ._core import load_hybrid_sentiment_train, CORPUS_SIZE
...
@click.group(help="Hybrid Sentiment Dataset Commandline")
def cli():
    pass
...
def download(access_key, secret_key, data_home):
    """Download hybrid sentiment corpus from cloud with your key."""

    data_home = Path(os.path.expanduser(data_home)) / 'hybrid_sentiment'
    data_home.mkdir(parents=True, exist_ok=True)
    console.print(f"Data directory for hybrid sentiment data {data_home}")
    ...
    url = f"s3://sadedegel/dataset/hybrid_sentiment_train.csv.gz"
...
def validate():
    ...
    with console.status("[bold yellow]Validating train"):
        train = load_hybrid_sentiment_train()
        ...
        if n_train == CORPUS_SIZE:
            ...
            console.log(f"|Hybrid sentiment (train)| : {n_train} ({CORPUS_SIZE} expected)")
            sys.exit(1)
